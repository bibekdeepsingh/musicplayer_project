
## Team Member
Bibekdeep Singh
Smilepreet Kaur
Navpreet Kaur

## Team Name
Group 5

## Project Description
This project is a simple music management app.
It allows users to view and organize playlists, subscriptions in a clean and accesible way.

## User Stories
1. As a user, I want to view my playlist, so that I can quickly see the songs I have saved.
2. As a user, I want to view my active music subscriptions, so that I can manage my monthly services.
3. As a user, I want the app to look clean and easy to use, so that I can enjoy interacting with it regularly.

## Components

Playlist Manager component was done by Bibekdeep Singh
subscription Manager component was done by Smilepreet Kaur